...
class Board(object):
	...
	def __init__(self, dimension, size, parent=None, row = None, col = None):
	...
	def move(self, player, *args):
		if len(args)!=2*self.dimension:
			raise Exception('move has wrong number of indices: should be %s, is %s' % (self.dimension*2, len(args))) #should never be thrown
		if self.state != -1:
		...
		if self.dimension==0:
			...
			self.children[args[0]][args[1]].move(player,*args[2:])
